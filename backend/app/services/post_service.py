...
def out_from(p: Post, viewer_id, like_count: int, comment_count: int, repost_count: int,
             is_liked: bool, is_saved: bool, is_following: bool, original: PostOut | None = None,
             vip_tier: str | None = None) -> PostOut:
    author = _author(p.author, p.author_id)
    author.vip_tier = vip_tier
    return PostOut(
        id=str(p.id), author=author,
...
async def to_out(db: AsyncSession, p: Post, viewer_id: uuid.UUID | None, depth: int = 1) -> PostOut:
    ...
    try:
        ...
        author.vip_tier = (await _mon.active_vip_map(db, [p.author_id])).get(p.author_id)
    except Exception:
        pass
    return PostOut(
        id=str(p.id), author=author,
...
async def create_post(db: AsyncSession, author_id: uuid.UUID, data: PostCreate) -> Post:
    ...
    if data.repost_of:
        ...
        if orig and orig.author_id != author_id:
            ...
            _t = f' "{orig.title}"' if orig.title else ""
            notification_service.add(db, orig.author_id, "repost", _nm, f"reposted your post{_t}", f"/post/{orig.id}", _post_image(orig), _av)
    await db.commit()
...
async def toggle_like(db: AsyncSession, post_id: uuid.UUID, user_id: uuid.UUID):
    ...
    if row:
        ...
        if p and p.author_id != user_id:
            ...
            _t = f' "{p.title}"' if p.title else ""
            notification_service.add(db, p.author_id, "post_like", _nm, f"liked your post{_t}", f"/post/{post_id}", _post_image(p), _av)
    await db.commit()
...
async def add_comment(db: AsyncSession, post_id: uuid.UUID, author_id: uuid.UUID, body: str, parent_id: str | None = None) -> PostCommentOut:
    ...
    if parent_id:
        ...
        if parent:
            ...
            if parent.author_id and parent.author_id != author_id:
                _nm, _av = await _actor(db, author_id)
                notification_service.add(db, parent.author_id, "reply", _nm, "replied to your comment", f"/post/{post_id}", None, _av)
    ...
    if p and p.author_id != author_id and not pid:
        _nm, _av = await _actor(db, author_id)
        _t = f' "{p.title}"' if p.title else ""
        notification_service.add(db, p.author_id, "post_comment", _nm, f"commented on your post{_t}", f"/post/{post_id}", _post_image(p), _av)
    await db.commit()
