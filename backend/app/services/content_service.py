...
async def get_detail(db: AsyncSession, content_id: uuid.UUID, current_user_id: uuid.UUID | None = None, current_anon_id: str | None = None):
    ...
    # Everything below is derived/optional — never let it 500 a found content.
    tags: list[str] = []
    like_count = follower_count = comment_count = 0
    is_liked = is_disliked = is_following = is_saved = False
    try:
        import logging as _logging  # noqa: F401
        from app.models import Comment, Dislike as _Dislike, Follow, Like
        ...
        if current_user_id or current_anon_id:
            ...
            is_liked = (await db.execute(
                select(Like).where(Like.content_id == content_id, like_w)
            )).scalar_one_or_none() is not None
            is_disliked = (await db.execute(
                select(_Dislike).where(_Dislike.content_id == content_id, dis_w)
            )).scalar_one_or_none() is not None
        if current_user_id:
            is_following = (await db.execute(
                select(Follow).where(Follow.follower_id == current_user_id, Follow.creator_id == c.owner_id)
            )).scalar_one_or_none() is not None
            from app.models import Save
            is_saved = (await db.execute(
                select(Save).where(Save.user_id == current_user_id, Save.content_id == content_id)
            )).scalar_one_or_none() is not None
    ...
    if current_user_id:
        try:
            ...
            if wh:
                ...
                db.add(WatchHistory(user_id=current_user_id, content_id=content_id))
            await db.commit()
        except Exception:
            await db.rollback()
