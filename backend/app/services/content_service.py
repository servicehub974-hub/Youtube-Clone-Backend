...
async def get_detail(db: AsyncSession, content_id: uuid.UUID, current_user_id: uuid.UUID | None = None):
    ...
    tres = await db.execute(
        select(Tag).join(_CT, _CT.tag_id == Tag.id).where(_CT.content_id == content_id)
    )
    ...
    if current_user_id:
        lk = await db.execute(
            select(Like).where(Like.content_id == content_id, Like.user_id == current_user_id)
        )
        is_liked = lk.scalar_one_or_none() is not None
        from app.models import Dislike as _Dislike
        dk = await db.execute(
            select(_Dislike).where(_Dislike.content_id == content_id, _Dislike.user_id == current_user_id)
        )
        is_disliked = dk.scalar_one_or_none() is not None
        fl = await db.execute(
            select(Follow).where(
                Follow.follower_id == current_user_id, Follow.creator_id == c.owner_id
            )
        )
...
async def toggle_like(
    ...
    dl = await db.execute(
        select(Dislike).where(Dislike.content_id == content_id, Dislike.user_id == user_id)
    )
    drow = dl.scalar_one_or_none()
    if drow:
        await db.delete(drow)

    existing = await db.execute(
        select(Like).where(Like.content_id == content_id, Like.user_id == user_id)
    )
    ...
    if row:
        ...
        db.add(Like(content_id=content_id, user_id=user_id))
        liked = True
    await db.commit()
...
async def toggle_dislike(
    ...
    if row:
        ...
        db.add(Dislike(content_id=content_id, user_id=user_id))
        disliked = True
    await db.commit()
    return disliked, await _count(db, Like, content_id=content_id)


async def list_by_owner_public(db: AsyncSession, owner_id: uuid.UUID, limit: int = 60):
    res = await db.execute(
        select(Content)
        .where(
