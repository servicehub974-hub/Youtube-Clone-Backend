...
async def reels_feed(db: AsyncSession, current_user_id, current_anon_id, cursor: str | None = None, limit: int = 6):
    from app.models import Comment, Follow, Like
    stmt = (select(Content).where(
        Content.is_short.is_(True), Content.status == "published", Content.visibility == "public"
    ).order_by(Content.created_at.desc()).limit(limit + 1))
    if cursor:
        try:
            stmt = stmt.where(Content.created_at < datetime.fromisoformat(cursor))
        except ValueError:
            pass
    rows = list((await db.execute(stmt)).scalars().all())
    has_more = len(rows) > limit
    ...
    items = []
    for c in rows:
        like_count = await _count(db, Like, content_id=c.id)
        comment_count = await _count(db, Comment, content_id=c.id)
        is_liked = is_following = False
        if current_user_id or current_anon_id:
            lw = (Like.user_id == current_user_id) if current_user_id else (Like.anon_id == current_anon_id)
            is_liked = (await db.execute(select(Like.content_id).where(Like.content_id == c.id, lw))).first() is not None
        if current_user_id:
            is_following = (await db.execute(select(Follow.creator_id).where(Follow.follower_id == current_user_id, Follow.creator_id == c.owner_id))).first() is not None
        locked = False
        if c.is_premium and c.price_gems and c.owner_id != current_user_id:
        ...
        items.append({
            ...
            "is_locked": locked, "creator": card.creator.model_dump(), "creator_id": str(c.owner_id),
            "like_count": like_count, "comment_count": comment_count, "is_liked": is_liked, "is_following": is_following,
        })
