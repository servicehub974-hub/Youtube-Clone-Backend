...
async def reels_feed(db: AsyncSession, current_user_id, current_anon_id, cursor: str | None = None, limit: int = 6):
    ...
    if current_user_id and ids:
        ...
        saved = set((await db.execute(select(_Save.content_id).where(_Save.user_id == current_user_id, _Save.content_id.in_(ids)))).scalars().all())
    try:
        from app.services import monetization_service as _mon
    ...
    for _, pid in page:
        ...
        items.append({
            ...
            "is_liked": (c.id in liked), "is_following": (c.owner_id in following), "is_saved": (c.id in saved),
        })
