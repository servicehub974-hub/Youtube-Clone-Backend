...
def _author(c: Comment) -> CommentAuthor:
    ...
    try:
        ...
        if parent is not None:
            if parent.user_id and parent.user_id != actor.user_id:
                notification_service.add(db, parent.user_id, "reply", name, "replied to your comment", link)
        else:
            owner = (await db.execute(select(Content.owner_id).where(Content.id == content_id))).scalar_one_or_none()
            if owner and owner != actor.user_id:
                notification_service.add(db, owner, "comment", name, "commented on your content", link)
    except Exception:
        pass
    await db.commit()
    ...
    if liked:
        try:
            ...
            if c and c.user_id and c.user_id != actor.user_id:
                notification_service.add(db, c.user_id, "comment_like", _actor_name(actor), "liked your comment", f"/content/{c.content_id}")
        except Exception:
            pass
    await db.commit()
