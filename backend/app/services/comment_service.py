...
async def create_comment(
    ...
    try:
        ...
        trow = (await db.execute(select(Content.owner_id, Content.thumbnail_url).where(Content.id == content_id))).first()
        thumb = trow[1] if trow else None
        if parent is not None:
            if parent.user_id and parent.user_id != actor.user_id:
                notification_service.add(db, parent.user_id, "reply", name, "replied to your comment", link, thumb)
        else:
            owner = trow[0] if trow else None
            if owner and owner != actor.user_id:
                notification_service.add(db, owner, "comment", name, "commented on your content", link, thumb)
    except Exception:
        pass
    await db.commit()
    ...
    if liked:
        try:
            ...
            if c and c.user_id and c.user_id != actor.user_id:
                ...
                notification_service.add(db, c.user_id, "comment_like", _actor_name(actor), "liked your comment", f"/content/{c.content_id}", _thumb)
        except Exception:
            pass
    await db.commit()
