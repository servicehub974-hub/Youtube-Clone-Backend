...
async def get_balance(db: AsyncSession, user_id: uuid.UUID) -> int:
    r = await db.execute(text("select gems from wallets where user_id = cast(:u as uuid)"), {"u": str(user_id)})
...
async def _credit(db: AsyncSession, user_id: uuid.UUID, delta: int, reason: str, ref: str | None = None):
    ...
    await db.execute(text("insert into wallets (user_id, gems) values (cast(:u as uuid), 0) on conflict (user_id) do nothing"),
                     {"u": str(user_id)})
    await db.execute(text("update wallets set gems = gems + :d, updated_at = now() where user_id = cast(:u as uuid)"),
...
async def vip_status(db: AsyncSession, user_id: uuid.UUID):
    row = (await db.execute(text(
        ...
        "where user_id = cast(:u as uuid) and (expires_at is null or expires_at > now()) "
...
async def approve_order(db: AsyncSession, order_id: uuid.UUID, admin_id: uuid.UUID):
    ...
    try:
        ...
        try:
            ...
            if o.kind == "gems":
                notification_service.add(db, o.user_id, "order", None, f"Your {o.amount_gems or ''} Gems order was approved 🎉", "/gems")
            elif o.kind == "vip":
                notification_service.add(db, o.user_id, "order", None, f"Your {o.item.upper()} VIP is now active 👑", "/vip")
            else:
                notification_service.add(db, o.user_id, "order", None, "Your order was approved", "/vip")
        except Exception:
            pass
        await db.commit()
...
async def claim_daily(db: AsyncSession, user_id: uuid.UUID):
    ...
    today = date.today()
    exists = (await db.execute(text("select 1 from daily_claims where user_id = cast(:u as uuid) and day = :d"),
...
async def claimed_today(db: AsyncSession, user_id: uuid.UUID) -> bool:
    return (await db.execute(text("select 1 from daily_claims where user_id = cast(:u as uuid) and day = :d"),
...
# ---------- premium content unlock (spend Gems, atomic) ----------
async def is_unlocked(db: AsyncSession, user_id, content_id) -> bool:
    from app.models import ContentUnlock
    if not user_id:
        return False
    return (await db.execute(select(ContentUnlock.content_id).where(
        ContentUnlock.user_id == user_id, ContentUnlock.content_id == content_id))).first() is not None


async def unlock_content(db: AsyncSession, user_id, content_id):
    from app.models import Content, ContentUnlock
    c = (await db.execute(select(Content).where(Content.id == content_id))).scalar_one_or_none()
    if not c:
        raise MonetizationError("Content not found")
    if not c.is_premium or not c.price_gems:
        return {"unlocked": True}  # free content — nothing to pay
    if c.owner_id == user_id:
        return {"unlocked": True}  # owner
    if await is_unlocked(db, user_id, content_id):
        return {"unlocked": True}
    bal = await get_balance(db, user_id)
    if bal < c.price_gems:
        raise MonetizationError(f"Not enough Gems. Need {c.price_gems}, you have {bal}.")
    try:
        await _credit(db, user_id, -c.price_gems, "content_unlock", str(content_id))
        if c.owner_id and c.owner_id != user_id:
            await _credit(db, c.owner_id, c.price_gems, "content_sale", str(content_id))
        db.add(ContentUnlock(user_id=user_id, content_id=content_id))
        await db.commit()
    except Exception as exc:  # noqa: BLE001
        await db.rollback()
        raise MonetizationError(f"Unlock failed: {exc}")
    try:
        if c.owner_id and c.owner_id != user_id:
            u = (await db.execute(select(Profile).where(Profile.id == user_id))).scalar_one_or_none()
            uname = (u.display_name or u.username or "Someone") if u else "Someone"
            uav = u.avatar_url if (u and u.avatar_url) else None
            ttl = f' "{c.title}"' if c.title else ""
            from app.services import notification_service
            await notification_service.notify(db, c.owner_id, "purchase", uname, f"unlocked your premium content{ttl}", f"/content/{content_id}", c.thumbnail_url, uav)
    except Exception:
        await db.rollback()
    return {"unlocked": True, "balance": await get_balance(db, user_id)}


async def active_vip_map(db: AsyncSession, user_ids) -> dict:
    """Batch: {user_id: tier} for users with an active VIP subscription."""
    ...
    rows = (await db.execute(
        select(VipSubscription.user_id, VipSubscription.tier).where(
            VipSubscription.user_id.in_(ids),
            or_(VipSubscription.expires_at.is_(None), VipSubscription.expires_at > now),
        )
    )).all()
    return {r[0]: r[1] for r in rows}


async def check_expiry(db: AsyncSession, user_id: uuid.UUID):
    """Notify (once) when a VIP subscription has just expired. Lazy — call on load."""
    now = datetime.now(timezone.utc)
    rows = (await db.execute(select(VipSubscription).where(
        VipSubscription.user_id == user_id,
        VipSubscription.expires_at.isnot(None),
        VipSubscription.expires_at < now,
        VipSubscription.expiry_notified.is_(False),
    ))).scalars().all()
    if not rows:
        return
    from app.services import notification_service
    for sub in rows:
        notification_service.add(db, user_id, "vip_expired", None, f"Your {sub.tier.upper()} VIP has expired", "/vip")
