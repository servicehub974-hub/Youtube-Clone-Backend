        ...
        if r.target_type == "content":
            ...
            if c:
                ...
                try:
                    ...
                    await notification_service.notify(db, c.owner_id, "moderation", "Moderator", f'Your content "{c.title}" was hidden after a report', f"/content/{c.id}", c.thumbnail_url)
                except Exception:
                    pass
        elif r.target_type == "comment":
...
async def restore_content(db: AsyncSession, admin: Profile, content_id: uuid.UUID):
    ...
    try:
        from app.services import notification_service
        await notification_service.notify(db, c.owner_id, "moderation", "Moderator", f'Your content "{c.title}" was restored', f"/content/{c.id}", c.thumbnail_url)
    except Exception:
        pass
    await db.commit()
    return {"ok": True}
