import uuid

from sqlalchemy import select, text
from sqlalchemy.ext.asyncio import AsyncSession

from app.models import Profile, Role
from app.schemas.user import ProfileUpdate, UserOut


class UserError(Exception):
    pass


def to_user_out(profile: Profile) -> UserOut:
    return UserOut(
        id=str(profile.id),
        email=getattr(profile, "email", None),
        username=profile.username,
        display_name=profile.display_name,
        avatar_url=profile.avatar_url,
        bio=profile.bio,
        banned=getattr(profile, 'banned', False),
        role=profile.role.name if profile.role else "user",
    )


async def _get(db: AsyncSession, user_id: uuid.UUID) -> Profile | None:
    res = await db.execute(select(Profile).where(Profile.id == user_id))
    return res.scalar_one_or_none()


async def get_or_create_profile(
    db: AsyncSession, user_id_str: str, email: str | None
) -> Profile:
    """Fetch the profile (created by the DB trigger on signup). Falls back to
    creating one if it's somehow missing."""
    user_id = uuid.UUID(user_id_str)
    profile = await _get(db, user_id)

    if profile is None:
        base = (email.split("@")[0] if email else "user")
        profile = Profile(
            id=user_id,
            username=f"{base}-{str(user_id)[:5]}",
            display_name=base,
            role_id=1,
        )
        db.add(profile)
        try:
            await db.commit()
        except Exception:
            await db.rollback()  # trigger created it concurrently
        profile = await _get(db, user_id)

    profile.email = email  # transient, for serialization only
    return profile


async def update_profile(
    db: AsyncSession, profile: Profile, data: ProfileUpdate
) -> Profile:
    for field, value in data.model_dump(exclude_unset=True).items():
        setattr(profile, field, value)
    await db.commit()
    refreshed = await _get(db, profile.id)
    assert refreshed is not None
    refreshed.email = getattr(profile, "email", None)
    return refreshed


async def _role_id(db: AsyncSession, role_name: str) -> int:
    res = await db.execute(select(Role.id).where(Role.name == role_name))
    rid = res.scalar_one_or_none()
    if rid is None:
        raise UserError(f"Unknown role: {role_name}")
    return rid


async def set_role(db: AsyncSession, user_id: uuid.UUID, role_name: str) -> Profile:
    profile = await _get(db, user_id)
    if not profile:
        raise UserError("User not found")
    profile.role_id = await _role_id(db, role_name)
    await db.commit()
    return await _get(db, user_id)  # type: ignore[return-value]


async def become_creator(db: AsyncSession, profile: Profile) -> Profile:
    if profile.role and profile.role.name == "admin":
        return profile  # don't downgrade admins
    profile.role_id = await _role_id(db, "creator")
    await db.commit()
    updated = await _get(db, profile.id)
    assert updated is not None
    updated.email = getattr(profile, "email", None)
    return updated


async def list_profiles(db: AsyncSession, limit: int = 50) -> list[Profile]:
    res = await db.execute(
        select(Profile).order_by(Profile.created_at.desc()).limit(limit)
    )
    return list(res.scalars().all())


async def toggle_follow(
    db: AsyncSession, follower_id: uuid.UUID, creator_id: uuid.UUID
) -> tuple[bool, int]:
    from sqlalchemy import func
    from app.models import Follow

    if follower_id == creator_id:
        raise UserError("You can't follow yourself.")

    existing = await db.execute(
        select(Follow).where(
            Follow.follower_id == follower_id, Follow.creator_id == creator_id
        )
    )
    row = existing.scalar_one_or_none()
    if row:
        await db.delete(row)
        following = False
    else:
        db.add(Follow(follower_id=follower_id, creator_id=creator_id))
        following = True
    await db.commit()
    cnt = await db.execute(
        select(func.count()).select_from(Follow).where(Follow.creator_id == creator_id)
    )
    return following, int(cnt.scalar_one())


async def get_public_profile(
    db: AsyncSession, user_id: uuid.UUID, current_user_id: uuid.UUID | None
):
    from sqlalchemy import func
    from app.models import Content, Follow
    from app.schemas.user import UserPublicOut

    p = await _get(db, user_id)
    if not p:
        return None

    fc = await db.execute(
        select(func.count()).select_from(Follow).where(Follow.creator_id == user_id)
    )
    tv = await db.execute(
        select(func.coalesce(func.sum(Content.views), 0)).where(
            Content.owner_id == user_id,
            Content.status == "published",
            Content.visibility == "public",
        )
    )
    cc = await db.execute(
        select(func.count()).select_from(Content).where(
            Content.owner_id == user_id,
            Content.status == "published",
            Content.visibility == "public",
        )
    )
    is_following = False
    if current_user_id:
        fl = await db.execute(
            select(Follow).where(
                Follow.follower_id == current_user_id, Follow.creator_id == user_id
            )
        )
        is_following = fl.scalar_one_or_none() is not None

    is_blocked = False
    try:
        if current_user_id:
            from app.models import UserBlock
            is_blocked = (await db.execute(select(UserBlock).where(UserBlock.blocker_id == current_user_id, UserBlock.blocked_id == user_id))).first() is not None
    except Exception:
        is_blocked = False

    vip_tier = None
    try:
        from app.services import monetization_service as _mon
        st = await _mon.vip_status(db, user_id)
        vip_tier = st.get("tier") if st.get("active") else None
    except Exception:
        vip_tier = None

    return UserPublicOut(
        id=str(p.id),
        username=p.username,
        display_name=p.display_name,
        avatar_url=p.avatar_url,
        cover_url=p.cover_url,
        bio=p.bio,
        role=p.role.name if p.role else "user",
        website=p.website,
        location=p.location,
        links=p.links or [],
        follower_count=int(fc.scalar_one()),
        content_count=int(cc.scalar_one()),
        total_views=int(tv.scalar_one()),
        joined=p.created_at.isoformat() if p.created_at else None,
        is_following=is_following,
        vip_tier=vip_tier,
        is_blocked=is_blocked,
    )


async def merge_anon(db: AsyncSession, user_id: uuid.UUID, anon_id: str) -> None:
    """Move a guest's anon likes/dislikes/comment-likes/comments to the account,
    dropping rows that would duplicate the user's existing reactions."""
    params = {"u": user_id, "a": anon_id}
    # likes
    await db.execute(text("delete from likes l where l.anon_id=:a and exists (select 1 from likes x where x.content_id=l.content_id and x.user_id=:u)"), params)
    await db.execute(text("update likes set user_id=:u, anon_id=null where anon_id=:a"), params)
    # dislikes
    await db.execute(text("delete from dislikes l where l.anon_id=:a and exists (select 1 from dislikes x where x.content_id=l.content_id and x.user_id=:u)"), params)
    await db.execute(text("update dislikes set user_id=:u, anon_id=null where anon_id=:a"), params)
    # comment likes
    await db.execute(text("delete from comment_likes l where l.anon_id=:a and exists (select 1 from comment_likes x where x.comment_id=l.comment_id and x.user_id=:u)"), params)
    await db.execute(text("update comment_likes set user_id=:u, anon_id=null where anon_id=:a"), params)
    # comments
    await db.execute(text("update comments set user_id=:u, anon_id=null, anon_name=null where anon_id=:a"), params)
    await db.commit()


async def has_anon_activity(db: AsyncSession, anon_id: str) -> bool:
    r = await db.execute(text(
        "select exists(select 1 from likes where anon_id=:a)"
        " or exists(select 1 from comments where anon_id=:a)"
        " or exists(select 1 from dislikes where anon_id=:a)"
        " or exists(select 1 from comment_likes where anon_id=:a)"
    ), {"a": anon_id})
    return bool(r.scalar())


async def list_following(db: AsyncSession, user_id: uuid.UUID, limit: int = 200):
    from app.models import Follow, Profile
    rows = (await db.execute(
        select(Profile).join(Follow, Follow.creator_id == Profile.id)
        .where(Follow.follower_id == user_id).limit(limit)
    )).scalars().all()
    return [
        {"id": str(p.id),
         "name": p.display_name or p.username or "User",
         "avatar": p.avatar_url or f"https://i.pravatar.cc/150?u={p.id}",
         "verified": bool(p.role and p.role.name in ("creator", "admin"))}
        for p in rows
    ]


async def toggle_block(db: AsyncSession, blocker_id, blocked_id) -> bool:
    from app.models import UserBlock
    if blocker_id == blocked_id:
        raise UserError("You can't block yourself.")
    row = (await db.execute(select(UserBlock).where(UserBlock.blocker_id == blocker_id, UserBlock.blocked_id == blocked_id))).scalar_one_or_none()
    if row:
        await db.delete(row); blocked = False
    else:
        db.add(UserBlock(blocker_id=blocker_id, blocked_id=blocked_id)); blocked = True
    await db.commit()
    return blocked


async def is_blocked_between(db: AsyncSession, a, b) -> bool:
    from app.models import UserBlock
    from sqlalchemy import and_, or_
    r = (await db.execute(select(UserBlock).where(or_(
        and_(UserBlock.blocker_id == a, UserBlock.blocked_id == b),
        and_(UserBlock.blocker_id == b, UserBlock.blocked_id == a),
    )))).first()
    return r is not None


async def list_blocked(db: AsyncSession, user_id):
    from app.models import UserBlock, Profile
    rows = (await db.execute(select(Profile).join(UserBlock, UserBlock.blocked_id == Profile.id).where(UserBlock.blocker_id == user_id))).scalars().all()
    return [{"id": str(p.id), "name": p.display_name or p.username or "User", "avatar": p.avatar_url or f"https://i.pravatar.cc/150?u={p.id}"} for p in rows]
