...
def add(session: AsyncSession, recipient_id, ntype: str, actor_name: str | None, message: str, link: str | None, image: str | None = None):
    """Queue a notification on the session (caller commits). No self-notify."""
    if recipient_id is None:
        return
    session.add(Notification(
        user_id=recipient_id, type=ntype,
        actor_name=actor_name or "Someone", message=message, link=link, image=image,
    ))


async def notify(db: AsyncSession, recipient_id, ntype, actor_name, message, link, image=None):
    """Standalone notify (own commit)."""
    if recipient_id is None:
        return
    try:
        add(db, recipient_id, ntype, actor_name, message, link, image)
        await db.commit()
    except Exception:
        await db.rollback()
...
async def list_notifications(db: AsyncSession, user_id: uuid.UUID, limit: int = 40):
    ...
    return [
        {"id": str(n.id), "type": n.type, "actor_name": n.actor_name,
         "message": n.message, "link": n.link, "image": n.image, "is_read": n.is_read,
