...
def add(session: AsyncSession, recipient_id, ntype: str, actor_name: str | None, message: str, link: str | None, image: str | None = None, actor_avatar: str | None = None):
    """Queue a notification on the session (caller commits). No self-notify."""
    if recipient_id is None:
        return
    session.add(Notification(
        user_id=recipient_id, type=ntype,
        actor_name=actor_name or "Someone", message=message, link=link, image=image, actor_avatar=actor_avatar,
    ))


async def notify(db: AsyncSession, recipient_id, ntype, actor_name, message, link, image=None, actor_avatar=None):
    """Standalone notify (own commit)."""
    if recipient_id is None:
        return
    try:
        add(db, recipient_id, ntype, actor_name, message, link, image, actor_avatar)
        await db.commit()
    except Exception:
        await db.rollback()
...
async def list_notifications(db: AsyncSession, user_id: uuid.UUID, limit: int = 40):
    ...
    return [
        {"id": str(n.id), "type": n.type, "actor_name": n.actor_name,
         "message": n.message, "link": n.link, "image": n.image, "actor_avatar": n.actor_avatar, "is_read": n.is_read,
