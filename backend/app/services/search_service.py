...
async def search(
    ...
    sql = f"""
      select s.id from (
        select c.id, c.published_at, c.views, c.category_id,
          ...
          (case when c.title ilike any(:pats::text[]) then 3 else 0 end)
          + (case when exists(select 1 from content_tags ct join tags t on t.id = ct.tag_id
                              where ct.content_id = c.id and t.name ilike any(:pats::text[])) then 2 else 0 end)
          + (case when exists(select 1 from categories cat where cat.id = c.category_id
                              and cat.name ilike any(:pats::text[])) then 2 else 0 end)
          + (case when exists(select 1 from profiles p where p.id = c.owner_id
                              and (coalesce(p.display_name,'') ilike any(:pats::text[])
                                   or coalesce(p.username,'') ilike any(:pats::text[]))) then 1 else 0 end)
          + (case when coalesce(c.description,'') ilike any(:pats::text[]) then 1 else 0 end)
          as score
        from content c
