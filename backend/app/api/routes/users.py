...
async def login_event(request: Request, user: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    from app.models import LoginEvent
    ip = request.headers.get("x-forwarded-for", "").split(",")[0].strip() or (request.client.host if request.client else None)
    ua = request.headers.get("user-agent", "")[:300]
    db.add(LoginEvent(user_id=user.id, ip=ip, user_agent=ua))
    await db.commit()
    return {"ok": True}


@router.get("/me/logins")
async def my_logins(user: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    from app.models import LoginEvent
    from sqlalchemy import select as _sel
    rows = (await db.execute(_sel(LoginEvent).where(LoginEvent.user_id == user.id).order_by(LoginEvent.created_at.desc()).limit(20))).scalars().all()
    return [{"ip": e.ip, "user_agent": e.user_agent, "at": e.created_at.isoformat() if e.created_at else None} for e in rows]
