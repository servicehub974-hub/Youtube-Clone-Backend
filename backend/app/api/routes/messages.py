...
async def edit_message(conv_id: str, msg_id: str, data: SendIn, me: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
        return await message_service.edit_message(db, uuid.UUID(conv_id), uuid.UUID(msg_id), me.id, data.body)
    except MessageError as e:
        raise HTTPException(400, detail=str(e))


@router.delete("/{conv_id}/messages/{msg_id}")
async def delete_message(conv_id: str, msg_id: str, me: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
        await message_service.delete_message(db, uuid.UUID(conv_id), uuid.UUID(msg_id), me.id)
    except MessageError as e:
        raise HTTPException(400, detail=str(e))
    return {"ok": True}
