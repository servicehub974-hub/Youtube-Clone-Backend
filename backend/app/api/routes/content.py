...
from app.services import feature_service, monetization_service
...
async def list_content(
    ...
    return await cached(f"feed:{limit}:{cursor}:{category}", 15, build)
...
async def view_content(
    ...
    await content_service.record_view(db, uuid.UUID(content_id), actor.user_id)
    return {"ok": True}


@router.get("/content/trending", response_model=list[ContentCardOut])
async def trending(db: AsyncSession = Depends(get_db)):
    from app.core.cache import cached
    return await cached("content:trending", 30, lambda: content_service.trending_feed(db))


@router.get("/reels")
async def reels(cursor: str | None = None, limit: int = Query(6, ge=1, le=30), actor: Actor = Depends(get_actor), db: AsyncSession = Depends(get_db)):
    try:
        items, nxt = await content_service.reels_feed(db, actor.user_id, actor.anon_id, cursor, limit)
        return {"items": items, "next_cursor": nxt}
    except Exception:
        import logging
        logging.getLogger("reels").exception("reels_feed failed")
        await db.rollback()
        return {"items": [], "next_cursor": None}


@router.post("/content/{content_id}/unlock")
async def unlock_content(content_id: str, user: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
        return await monetization_service.unlock_content(db, user.id, uuid.UUID(content_id))
...
async def manage_content(user: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return await content_service.list_manage(db, user.id)


@router.get("/content/saved", response_model=list[ContentCardOut])
async def saved_content(
    user: Profile = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await content_service.list_saved(db, user.id)


@router.get("/content/history", response_model=list[ContentCardOut])
async def history_content(
    user: Profile = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await content_service.list_history(db, user.id)


@router.get("/content/liked", response_model=list[ContentCardOut])
async def liked_content(
    user: Profile = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await content_service.list_liked(db, user.id)
...
async def save_content(
    ...
    saved = await content_service.toggle_save(db, user.id, uuid.UUID(content_id))
    return {"saved": saved}
