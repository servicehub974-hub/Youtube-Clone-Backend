...
@router.get("/reels")
async def reels(cursor: str | None = None, actor: Actor = Depends(get_actor), db: AsyncSession = Depends(get_db)):
    items, nxt = await content_service.reels_feed(db, actor.user_id, actor.anon_id, cursor)
