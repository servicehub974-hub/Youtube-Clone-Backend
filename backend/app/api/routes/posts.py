...
async def comments(post_id: str, viewer: Profile | None = Depends(get_current_user_optional), db: AsyncSession = Depends(get_db)):
    return await post_service.list_comments(db, uuid.UUID(post_id), viewer.id if viewer else None)
...
async def add_comment(post_id: str, data: PostCommentIn, user: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return await post_service.add_comment(db, uuid.UUID(post_id), user.id, data.body, data.parent_id)


@router.post("/comments/{comment_id}/like")
async def like_comment(comment_id: str, user: Profile = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    liked, count = await post_service.toggle_comment_like(db, uuid.UUID(comment_id), user.id)
    return {"liked": liked, "count": count}
