...
async def search_content(
    ...
    try:
        ...
        import logging
        logging.getLogger("search").exception("search failed")
        await db.rollback()
        cards, count = [], 0
    posts = []
    if q.strip():
        try:
            posts = await search_service.search_posts(db, q, actor.user_id)
        except Exception:
            await db.rollback()
        try:
            await search_service.record(db, actor.user_id, actor.anon_id, q, count + len(posts))
        except Exception:
            await db.rollback()
    return {"items": [c.model_dump() for c in cards], "count": count, "posts": [p.model_dump() for p in posts]}
